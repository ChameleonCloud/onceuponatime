...
database = MySQLDatabase('nova', **{'host': '127.0.0.1', 'port': 3306, 'user': 'readonly'})
...
class ComputeNodes(BaseModel):
    cpu_allocation_ratio = FloatField(null=True)
    cpu_info = TextField()
    created_at = DateTimeField(null=True)
    ...
    ram_allocation_ratio = FloatField(null=True)
...
class InstanceExtra(BaseModel):
    ...
    migration_context = TextField(null=True)
    numa_topology = TextField(null=True)
...
class Migrations(BaseModel):
    ...
    instance_uuid = ForeignKeyField(db_column='instance_uuid', null=True, rel_model=Instances, to_field='uuid')
    migration_type = CharField(null=True)
...
class Services(BaseModel):
    ...
    forced_down = IntegerField(null=True)
    host = CharField(null=True)
    last_seen_up = DateTimeField(null=True)
...
class VirtualInterfaces(BaseModel):
    ...
    network = IntegerField(db_column='network_id', index=True, null=True)
    updated_at = DateTimeField(null=True)
    uuid = CharField(index=True, null=True)
